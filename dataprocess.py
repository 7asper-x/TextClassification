...
data_test = pd.read_csv(r'e:/train_poi.csv', encoding='utf8')
x_train = data['type3']
y_train = [[str(i)] for i in data['二级土地_1']]
x_test = data_test['type3']
y_test = [[str(i)] for i in data_test['二级土地_1']]
print(x_test)
s = DataPreprocess()

cutted = s.cut_texts(x_test)
s.train_tokenizer(cutted)
seq = s.text2seq(cutted)
...
s.creat_label_set(y_test)
labels = s.creat_labels(y_test)
for i in labels:
    print(i)
